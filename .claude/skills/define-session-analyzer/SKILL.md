...
2. **Drop machinery injected into the user slot.** Of what survives the first filter, around a quarter is still not a person: system reminders, task notifications, subagent task briefs, skill and command injection payloads, hook notices. Recognizable openers include `<system-reminder>`, `<task-notification>`, `[Subagent Task]`, `[Subagent Context]`, `Base directory for this skill:`, and `<local-command…>`. Where a line carries `origin.kind`, `human` confirms it — but that field is absent from most lines, so it can confirm a turn and never disqualify one.
