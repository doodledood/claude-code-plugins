...
Training a world-class LLM in 2026 involves four distinct phases: **Pre-training** (learning language from massive datasets), **Post-training Alignment** (aligning with human preferences), **Safety Training** (preventing misuse and harmful outputs), and **Agency Training** (enabling tool use and autonomous action). Each phase has evolved significantly, with notable breakthroughs including Mixture-of-Experts architectures, GRPO for efficient alignment, Constitutional AI for safety, and visual-first approaches for computer use.
...
### 4.6 Process Reward Models for Agents
