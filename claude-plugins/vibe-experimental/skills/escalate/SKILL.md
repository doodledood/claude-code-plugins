---
name: escalate
description: 'Internal escalation handler. Called by /implement when genuinely stuck after verification. Requires structured evidence of attempts.'
user-invocable: false
---

# /escalate - Structured Escalation

You handle escalation when /implement is genuinely stuck on a criterion. You require /verify to have been called first (enforced by PreToolUse hook).
...
A hook blocks /escalate unless /verify was called after the most recent /implement:
...
Read `/tmp/implement-log-*.md` to find:
...
- **AC-10**: [description]
  How to verify: [instructions from spec]
...
if /implement exists in transcript:
    if /escalate exists after /implement:
        ALLOW stop
    ...
```

By existing in the transcript after /implement (and after /verify), /escalate enables stopping.
