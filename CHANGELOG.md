...
- [consultant] v1.9.3 - Reformat `consultant_cli.py` to the repository's formatter. Two f-strings divided without spaces around the operator, which the formatter this repository configures has always wanted spaced; nothing ran it until `scripts/check.sh` existed, so the drift sat in the file. No behaviour change.
- **BREAKING**: [prompt-engineering] Plugin retired and removed from the marketplace. Prompt authoring and review already came from [manifest-dev](https://github.com/doodledood/manifest-dev) (v3.0.0, v4.0.0), leaving a plugin named for a capability it no longer provided. Gone with it: the `auto-optimize-prompt`, `compress-prompt` and `optimize-prompt-token-efficiency` skills and the `prompt-reviewer`, `prompt-compression-verifier` and `prompt-token-efficiency-verifier` agents. These have no upstream replacement — recovering one means reading it out of git history.
- **BREAKING**: [prompt-engineering] v4.0.0 - Removed the review-prompt skill; it now comes from [manifest-dev](https://github.com/doodledood/manifest-dev), which grew its own and maintains it. Same rule as v3.0.0 applied to the one skill that had since gained an upstream equivalent: where a capability exists upstream in a better-maintained form, this repository consumes it. The plugin keeps auto-optimize-prompt, compress-prompt, and optimize-prompt-token-efficiency, which have no upstream twin, plus its three agents — manifest-dev ships none. Also refreshes the plugin README and the two repository READMEs, which still advertised the prompt-engineering skill this plugin stopped shipping in v3.0.0.
- **BREAKING**: [prompt-engineering] v3.0.0 - Removed the prompt-engineering skill (moved to [manifest-dev](https://github.com/doodledood/manifest-dev), which ships a newer version: a thin SKILL.md with progressive disclosure into seven references, replacing the 242-line monolith and its three). The plugin keeps auto-optimize-prompt, compress-prompt, optimize-prompt-token-efficiency, and review-prompt; the prompt-reviewer agent now invokes the unscoped `prompt-engineering` skill, since it is no longer supplied by this plugin.
- 2026-05-05 [prompt-engineering] v2.6.1 - Clarify in the Success-vs-Constraints-vs-Stop boundary block that Constraints bound the path, not restate Success negatively (reverse-success-criteria failure mode); add operational test ("would the rule still apply if the goal changed?") with concrete examples.
- 2026-05-01 [prompt-engineering] v2.6.0 - Restructure prompt-engineering skill around the canonical seven-section system-prompt template (Role / Personality / Goal / Success criteria / Constraints / Output / Stop rules); add references/system-prompt-patterns.md (verification loops, retrieval/tool budgets, output contract, ambiguity handling, high-risk self-check, decision-rules) and references/metaprompting.md (diagnose-from-failures → surgical-patch workflow). Skills and agents shown as specializations on top of the canonical template. Existing content preserved: Context Discovery, Anti-Patterns, Memento, Emotional Tone, Gotchas, Validation Checklist.
- [prompt-engineering] v2.5.1 - Refine prompt-engineering skill from GPT-5.5 guidance review: split "Weak language" anti-pattern into "Weak hedging" + "Absolutes for judgment calls" (reserve absolutes for true invariants; use decision rules for judgment calls); refactor System Instructions template (drop Approach slot, add Goal slot, broaden Success criteria with concrete guidance covering output correctness, validation passing, time/iteration bounds, and non-success decisions — retry/fallback/abstain/ask). Memento Pattern, Emotional Tone, and sibling skills unchanged.
- [writing] v1.3.0 - Fold Wikipedia:Signs of AI writing patterns into human-writing skill, ai-tells-and-fingerprints reference, and writing-reviewer agent. Adds era-tracked vocabulary (GPT-4 era vs GPT-4o era, snapshotted), curly-quote tell with model attribution, heading capitalization, excessive boldface ("key takeaways" pattern), encyclopedic-yet-promotional drift, "moreover" / "breathtaking" puffery, "myths busted" contrast pattern, subject puffery, statistical regression to mean, and Wikipedia-context-only signs (notability-language echoing, level-2 heading skipping, citation-template mimicry). Reviewer gains explicit frequency-aware-judgment guidance and context-scoped sign handling so Wikipedia-only tells don't false-positive on blogs / marketing / emails.
- [prompt-engineering] v2.5.0 - Add Emotional Tone section to prompt-engineering skill with research-grounded principles (arousal management, opening framing, failure normalization, sycophancy-harshness tradeoff, high-stakes framing) from transformer emotions research. Strip prompt-reviewer to thin wrapper that relies on loaded skill principles instead of duplicating them.
